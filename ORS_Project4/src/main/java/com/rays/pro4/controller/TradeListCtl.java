...
/**
 * Trade List functionality Controller. Performs operation for list, search
 * and delete operations of Trade
 * 
...
public class TradeListCtl extends BaseCtl {
	...
	protected void preload(HttpServletRequest request) {
		...
		TradeModel cmodel = new TradeModel();

		try {

			List clist = cmodel.list(0, 0);

			request.setAttribute("transactionType", clist);
		...
		if (OP_SEARCH.equalsIgnoreCase(op)) {
			...
			if (ids != null && ids.length > 0) {
				...
				for (String id : ids) { 
					...
					System.out.println("Trade Model id ===="+deletebean.getId());
					try {
					...
					ServletUtility.setSuccessMessage("Trade is Deleted Successfully", request);
				}
			} 
			else {
